	...
	*/
	...
	IEnumerator UpdateScores() {
		...
		while( true ){
           ...
           for (int i = 0; i < 40; i++)                     // On a augmenter la résolution de la texture prise pour calculer les scores à 40*40 
            {
                for (int j = 0; j < 40; j++)
                {
                    scoresColor += Tex4.GetPixel(i, j);
                }
                yield return new WaitForSeconds(0.01f);
            ...
            scores.x = scoresColor.r;                   // Le script a l'air de calculer les scores en fonction du RGB de la texture finale.
